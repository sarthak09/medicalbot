...
def initialize_rag():
    ...
    print(f"Loaded {len(dataloader.documents)} documents.", flush=True)
    splitter = Splitter(dataloader.documents)
    print(f"Split into {len(splitter.chunks)} chunks.", flush=True)
    ...
    print(f"Vector store created at {db_dir}.", flush=True)
