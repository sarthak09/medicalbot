...
```bash
docker ps
docker logs distracted_rhodes
```

If the password isn’t visible, run:

```bash
docker exec distracted_rhodes cat /var/jenkins_home/secrets/initialAdminPassword
```
...
```bash
docker exec -u root -it distracted_rhodes bash
apt update -y
